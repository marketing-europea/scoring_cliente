...
def current_state_dict() -> Dict[str, int]:
    return {
        f"sel_{v}": int(st.session_state.get(f"sel_{v}", 0))
        for v in VAR_LIST
    }
...
def fill_random_client(tipo: str, tries: int = 400) -> Tuple[bool, float, str]:
    ...
    if tipo == "A":
        ...
        for var in vars_shuffled:
            ...
            new_idx = max(candidates, key=lambda i: xs[i])
...
if view == "C. Ayuda":
    ...
    - Columnas con los mismos nombres que las variables del JSON.

    Los valores pueden introducirse como:

    - Etiqueta exacta de la categoría.
    - Índice numérico de la categoría, empezando en 0.
    ...
    Permite subir un archivo CSV o Excel con varios clientes.
    ...
    Permite seleccionar una categoría para cada variable y ver:

    - Score total.
    - Tipo asignado.
    - Contribución de cada variable.

    También incluye botones para generar ejemplos de clientes tipo A, B, C, D o E.

    ---

    ## Pantalla B: Resumen estratégico

    Genera ejemplos de clientes tipo A, B, C, D y E.

    Para cada tipo muestra:

    - Score de ejemplo.
    - Variables principales.
    - Drivers de contribución.
    ...
    **A ≥ B ≥ C ≥ D**
    ...
    1. Consultar esta ayuda si se necesita entender la app.
